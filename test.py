...
st.write('Furthermore, we can assume that this Random Forest model tends to favor the classcmajority low_risk due to the predominance of this class in allcdata, leading to poor model performance on the high_risk class. This must be rebalanced in order to be able to fairly predict these two classes.
This imbalance of the two classes may be due to too large a proportion of false positives for the high_risk class implying that the model tends to incorrectly classify the high_risk and low_risk classes. In other words, this means that individuals who have granted loans can be wrongly classified as high_risk/low_risk, thus potentially leading to inappropriate decisions. To fill these gaps, it is necessary to rebalance the data of the two target classes (high_risk and low_risk) in order to have a precise and fair model in the prediction of all classes. For example, it may be useful to increase the number of minority subcategories in the “high_risk” class in order to improve the precision of this class. Conversely, we can also reduce the number of majority “low_risk” subcategories in order to have a balance datasets.
On the other hand, it may be interesting to add adjustment weights between these two classes. In this case it would perhaps be necessary to grant a greater weight of error to the minority high_risk class than the majority low_risk class so that the model is “penalized” more for incorrect classifications of the high_risk class.
Additionally, it may also be useful to move from categorical binary categories to variables continuous data to better capture complex predictions and information that is difficult to categorize. This also helps avoid “forcing” predictions into a category.
specific binary and therefore reduce classification errors (particularly in cases limits which are found neither in high_risk nor in low_risk).
In short, the Random Forest model, although effective in predicting the “low_risk” class, demonstrates significant shortcomings in the prediction of the “high_risk” class, mainly due to very low precision. Improvements should therefore focus on balancing data and adjusting the model to better capture the nuances of the cases at hand high risk')
...
st.write('Looking at the confusion matrix table, the model missed a high number of borrowers by misclassifying them as low risk when they were actually high risk. This can have significant consequences on the lending activity of a financial institution. Indeed, the institution could be faced with significant losses which unfortunately could have been avoided. Additionally, the high proportion of false positives means that the model classifies many borrowers as high risk when they are actually low risk. This can lead to the rejection of a large number of creditworthy borrowers, resulting in lost revenue opportunities and reduced customer portfolio growth. Indeed, if a low-risk client is perceived as a high-risk client by an institution, he will not hesitate to look at the competition. On the other hand, it can indirectly harm customer relations, with a greater number of dissatisfied customers harming the credit institution. of dissatisfied customers harms the reputation of the credit institution and therefore customer loyalty. On the other hand, a model that outclasses high-risk customers without strong and viable justification may be problematic in the eyes of the law considering that such misclassifications may result in risks of discrimination, sanctions or litigation ( especially when customers feel wronged by unjustifiable refusals). However, we find that a minority of borrowers were correctly classified as presenting a low risk in comparison with the other values in the matrix. This suggests that the Random Forest model is relatively weak in identifying safe (i.e. low risk) borrowers. This may indicate an imbalance in the model which might need adjustment to better balance detection between the two risk categories. It is clear that the model needs to be improved in order to minimize the number of false negatives which generate a very high risk of potential loss for the financial institution, and to better identify high-risk borrowers. It is also necessary to minimize misclassification errors, which can have significant financial implications for the institution's lending activity. the lending activity of the institution.')
